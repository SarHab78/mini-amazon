...
class AddProductForm(FlaskForm):
   #id is generated by us, and the seller_id is current user id 
    ...
    describe = StringField(_l('Product Description'), validators=[DataRequired(), Email()])
    image_url = StringField(_l('Image URL'), validators=[DataRequired()])
    price = StringField(_l('Price'), validators=[DataRequired()])
    quantity = PasswordField(_l('Quantity'), validators=[DataRequired()])
    available = PasswordField(_l('Available'), validators=[DataRequired()])
...
def add_items():
    ...
    if form.validate_on_submit():
        if Product.add_product(form.product_name.data,
                         form.describe.data,
                         form.image_url.data,
                         form.price.data,
                         form.quantity.data,
