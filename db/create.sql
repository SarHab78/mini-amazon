...
CREATE VIEW Prod_Sell_Rev_Cat AS(
    ...
    PUR.firstname, PUR.lastname, PUR.is_seller, PUR.address, PUR.id, PUR.available, PUR.avg_rating, C.cat_name
    FROM(
        SELECT PU.product_name, PU.product_id, PU.product_description, PU.image_url, PU.price, PU.quantity,
        PU.firstname, PU.lastname, PU.is_seller, PU.address, PU.id, PU.available, ROUND(R.avg_rating, 1) AS avg_rating
        FROM (SELECT P.product_name, P.product_id, P.product_description, P.image_url, P.price, P.quantity, P.available,
            U.firstname, U.lastname, U.is_seller, U.address, U.id
